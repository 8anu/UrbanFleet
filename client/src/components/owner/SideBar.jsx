...
const SideBar = () => {
  ...
  return (
    <div className='relative min-h-screen md:flex flex-col items-cnter pt-8
    max-w-13 md:max-w-60 w-full border-r border-borderColor text-sm'>
      ...
      <div className='group relative'>
        <label htmlFor="image">
          <img src={image ? URL.createObjectURL(image) : user?.image ||
            "https://unsplash.com/photos/grayscale-photo-of-man-XHVpWcr5grQ"} alt="" 
      ...
      {image && (
        <button className='absoulte top-0 right-0 flex p-2 gap-1 bg-primary/10 text-primary cursor-pointer'>
      ...
      <div className='w-full'>
        {ownerMenuLinks.map((link,index)=>(
          <NavLink key={index} to={link.path} className={`relative flex items-center gap-2 -full py-3 pl-4 first:mt-6
           ${link.path === location.pathname ? 'bg-primary/10 text-primary' : 'text-gray-600'}`}>
            ...
            <div className={`${link.path === location.pathname && 'bg-primary'}
             w-1.h h-8 rounded-l right-0 absolute`}></div>
          </NavLink>
        ))}
      </div>
